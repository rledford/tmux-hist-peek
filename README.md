...
A tmux plugin that maintains a per-pane file history that any tool or script can push to, and lets you quickly pass those files to any command via a popup overlay.

## Requirements

- tmux
- [jq](https://jqlang.org)
- A tool or script feeding file paths into the history (see [Integrations](#integrations))
...
What ends up in the history is entirely up to what you integrate — recently read files, newly created files, files modified by an agent, or anything else.
